...
public class CatalogoEventListener {
	...
	@KafkaListener( topics = "${topic.customer.name:topic-catalogo-VERA}",
	...
	public void consumer(Event<?> event) {
		...
		if (event.getClass().isAssignableFrom(CatalogoCreateEvent.class)) {
			...
			Catalogo objCat = objEvent.getData();
			
			int idCatalogo = objCat.getIdCatalogo();
			String descripcion = objCat.getDescripcion();
			
			System.out.println(">>3  ID: " + id);
			System.out.println(">>4  Tipo de evento: " + tipoEvento);
			System.out.println(">>5  Fecha: " + fecha);
			System.out.println(">>6  ID Pais: " + idCatalogo);
